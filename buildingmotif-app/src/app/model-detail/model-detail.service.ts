...
export class ModelDetailService {
  ...
  getManifestLibraryImports(id: number) {
    return this.http.get<{library_ids: number[]}>(API_URL + `/models/${id}/manifest/imports`)
      .pipe(
        retry(3),
  ...
  saveManifest(id: number, library_ids: number[], selected_template_ids: number[]) {
    const headers = {'Content-Type': "application/json"};
    return this.http.post(
      API_URL + `/models/${id}/manifest/imports`,
      { library_ids, selected_template_ids },
