...
def test_get_model_cbd_endpoint_basic_and_self_contained(client, building_motif):
    ...
    res = client.get(f"/models/{model.id}/node_subgraph", query_string={"node": str(s), "self_contained": "false"})
    ...
    res2 = client.get(f"/models/{model.id}/node_subgraph", query_string={"node": str(s), "self_contained": "true"})
