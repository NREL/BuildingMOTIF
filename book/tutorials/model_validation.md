...
In this case, the generated templates are fairly simple; they just require an input which is the name of the supply fan.

We can loop through each of these generated templates and create a name. Here, we are creating arbitrary names that are easy to follow but in a real setting you would
likely pull the equipment/point names from an external source like a BACnet network or BIM.

```{code-cell}
supply_fan_number = 0
for templ in generated_templates.get_templates():
    supply_fan_number += 1
    supply_fan_name = BLDG[f"Supply-Fan-{supply_fan_number}"]
    # we know from our exploration above that each template has
    # exactly 1 parameter; the next line of code binds that parameter
    # to the name of the supply fan
    bindings = {
        templ.parameters.pop(): supply_fan_name
    }
