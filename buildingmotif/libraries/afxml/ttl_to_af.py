...
from rdflib import RDFS, RDF, Namespace, Graph
...
class Translator():
    ...
    def createAFTree(self, firstttl, outpath, merge=None):
        ...
        for subj, pred, obj in self.graph.triples((None, None, None)):
            if pred not in [RDF['type'], RDFS['label']]:
                ...
                if pred in [self.BRICK['hasTag'], self.BRICK["Max_Limit"], self.BRICK["Min_limit"], self.BRICK['hasUnit'], 
    self.BRICK['lastKnownValue']]:
                ...
                elif pred == self.BRICK['isPartOf'] or pred == self.BRICK['hasLocation']:
    ...
    def getParent(self, obj):
        ...
        for s, p, o in self.graph.triples((None, None, None)):
            if p in [self.BRICK["hasPart"], self.BRICK['hasPoint'], self.BRICK['isLocationOf']] and o == obj:
                return s
            elif p in [self.BRICK["isPartOf"], self.BRICK['isPointOf'], self.BRICK['hasLocation']] and s == obj:
                return o
